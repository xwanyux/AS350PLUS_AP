...
//	TL_DumpHexData( 0, 0, length, data );
    ...
    args[0] = g_mac_key_index;
    memmove(&args[1], &addr, 4);
    memmove(&args[1 + 4], &length, 4);
    memmove(&args[1 + 4 + 4], data, length);
    IPC_clientHandler(psDEV_SRED, 1, 4, 1 + 4 + 4 + length, args, &retval);
...
//	  TL_DumpHexData( 0, 0, len, data );
    ...
    IPC_clientHandler(psDEV_SRED, 2, 3, 4 + 4, args, &retval);
