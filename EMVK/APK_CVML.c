...
//    PP_GenPinBlock( pinblock );
    //   PED_GenPinBlock( pinblock );	// 2014-10-29
      api_ped_GenPinBlock(pinblock);    // modified by Tammy
...
#else
    // “pindata” is no use, ONLY pass “mode”, “pkm” and “pke” to IPC_server.
    
    // mode    - 0 = plaintext PIN format, 1 = enciphered PIN format
    // pkm     - public key modulus  (2L-V, available for mode 1)
    // pke     - public key exponent (V,    available for mode 1)

    UCHAR   retval;
    UINT    pkm_len = 2 + pkm[0] + pkm[1] * 256;
    UINT    tot_len = 1 + pkm_len + 3;
    UCHAR   args[tot_len];

    memmove( args, &mode, 1 );
    memmove( &args[1], pkm, pkm_len );
    memmove( &args[1 + pkm_len], pke, 3 );

    IPC_clientHandler( psDEV_PED, 12, 3, tot_len, args, &retval );
    
printf("@@@@@@@@@@@@%s %d\n",__func__,__LINE__);
printf("\CVM_VerifyPIN() retval= %x\n", retval);

    return( retval );      // ONLY return the result
#endif
}
...
//                             if LL=0, no online PIN data. (8/16 bytes if available)
//	         ksn       - LL-V, key serial number for online.  (fixed 12 bytes if available)
// REF     :
// RETURN  : apkOK
//           apkNotReady     ( PIN entry bypassed )
//           apkFailed       ( PIN Try Limit exceeded --> try next CVM entry )
...
UINT  iLen;
      ...
      // clear EPB & KSN data
      apk_ClearRamDataTERM( ADDR_TERM_EPIN_DATA, 18, 0x00 );
      apk_ClearRamDataTERM( ADDR_TERM_KSN, 14, 0x00 );
      ...
      if( result == apiOK )
        ...
        apk_WriteRamDataTERM( ADDR_TERM_EPIN_DATA, 18, epb );	// LL-V(16)
	apk_WriteRamDataTERM( ADDR_TERM_KSN, 14, ksn );		// LL-V(12)
