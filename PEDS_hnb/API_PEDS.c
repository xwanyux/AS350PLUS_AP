...
#include "POSAPI.h"
#include "PEDSAPI.h"
#include "OS_FLASH.h"			// same as defined in IPC_server
#include "OS_SECM.h"			// same as defined in IPC_server
//#include "OS_SECM2.h"			// the 2'nd secure memory for PEDS API (to be appended to OS_SECM.h in IPC_server)
...
#define	BSP_Malloc_EX		malloc
#define	BSP_Free_EX		free
...
// RETURN  : none.
// ---------------------------------------------------------------------------
void	PEDS_TripleDES( UCHAR *key, UCHAR len, UCHAR *idata, UCHAR *odata )
{
UCHAR	i, j;
UCHAR	cnt;
UCHAR	xkey[24];	// expanded to 24-bit key (L-R-L)
UCHAR	ibuf[8];
UCHAR	obuf[8];
...
// RETURN  : none.
// ---------------------------------------------------------------------------
void	PEDS_TripleDES2( UCHAR *key, UCHAR len, UCHAR *idata, UCHAR *odata )
{
UCHAR	i, j;
UCHAR	cnt;
UCHAR	xkey[24];	// expanded to 24-bit key (L-R-L)
...
// RETURN  : none.
// ---------------------------------------------------------------------------
void	PEDS_CBC_TripleDES( UCHAR *key, UCHAR *icv, UINT len, UCHAR *idata, UCHAR *odata )
{
UINT	i, j;
UINT	cnt;
UCHAR	ctext[8];
UCHAR	ptext[8];
        ...
        if( cnt )
          ...
          for( i=0; i<cnt; i++ )
             ...
             // DES( ptext, key(L) )
             api_des_encipher( ptext, ctext, key );
             
             // -DES( ctext, key(R) )
             api_des_decipher( ptext, ctext, &key[8] );
...
// RETURN  : none.
// ---------------------------------------------------------------------------
void	PEDS_CBC_TripleDES2( UCHAR *key, UCHAR *icv, UINT len, UCHAR *idata, UCHAR *odata )
{
UINT	i, j;
UINT	cnt;
UCHAR	ctext[8];
UCHAR	ptext[8];
        ...
        if( cnt )
          ...
          for( i=0; i<cnt; i++ )
             ...
             // -DES( ctext, key(L) )
             api_des_decipher( ptext, ctext, key );
             
             // DES( ptext, key(R) )
             api_des_encipher( ptext, ctext, &key[8] );
...
// INPUT   : deskey - DES or TDES key used to encrypt.
...
//           apiFailed
// ---------------------------------------------------------------------------
UCHAR   PEDS_GenMAC_BASE24( UCHAR *deskey, UCHAR *icv, UINT len, UCHAR *data, UCHAR *mac )
{
UINT    i, j;
UINT    cnt;
UINT    left_bytes;
...
UCHAR   buf[8];
        ...
        // --- ANSI X9.19 CBC Procedures (CTCB BASE24 ONLY) ---
        cnt = len / 8;
        left_bytes = len % 8;
        i = 0;
        ...
        if( left_bytes )
          {
          // padding method 1 (right-padded with 0x00)
          memset( buf, 0x00, 8 );
...
// ---------------------------------------------------------------------------
void	PEDS_LoadPAN12( UCHAR *ipan, UCHAR *opan )
{
UCHAR	i;
UCHAR	panlen;
UCHAR	len;
...
// ---------------------------------------------------------------------------
void	PEDS_GenPinBlock_ISO0( UCHAR *pindata, UCHAR *pandata, UCHAR *pinblock )
{
UCHAR	epinblock[16];
UINT	i, j;
UCHAR	nibble_h;
...
UCHAR kek[16];
      ...
      PEDS_TripleDES2( kek, 16, emkey, mkey );
}
...
//           FALSE - failed.
// ---------------------------------------------------------------------------
void  PEDS_EncryptMKEY( UCHAR length, UCHAR *mkey, UCHAR *emkey )
{
UCHAR kek[PED_MSKEY_SLOT_LEN];
      ...
      PEDS_TripleDES( kek, length, mkey, emkey );
}
...
UCHAR	i;
UCHAR	result;
UCHAR	buf[PED_MSKEY2_SLOT_LEN2];
UCHAR	mkey[PED_MSKEY2_SLOT_LEN2];
ULONG	FlashAddr = F_ADDR_PED_ETMK_01;
	...
	// If current TMSs are not available, retrieve them from FLASH
        for( i=0; i<MAX_MKEY2_CNT; i++ )
           {
           memset( mkey, 0x00, sizeof(mkey) );
           result = PEDS_ReadRamDataKEY( ADDR_PED_MKEY2_01+(i*PED_MSKEY2_SLOT_LEN2), PED_MSKEY2_SLOT_LEN2, mkey ); 
           if( result != apiOK )
             goto OPEN_EXIT;
             
           if( mkey[0] != PED_MSKEY2_LEN ) // MKEY available? (in SRAM)
             {          
             // retrieve ETMK
//           result = apk_ReadFlashData( ADDR_PED_MKEY, PED_MSKEY_SLOT_LEN, buf );
             FLASH_ReadData( (void *)FlashAddr+(i*PED_MSKEY2_SLOT_LEN2), buf, PED_MSKEY2_SLOT_LEN2 );
             if( buf[0] == PED_MSKEY2_LEN ) // MKEY available? (in FLASH)
               {
               // Restore default MKEY by default Key Encryption Key (KEK)
               mkey[0] = buf[0];
               PEDS_DecryptMKEY( &buf[1], &mkey[1] );
               
               // Restore KCV
               memmove( &mkey[PED_MSKEY2_SLOT_LEN], &buf[PED_MSKEY2_SLOT_LEN], 3 );
             
               // save it to SRAM key slot (L-V-KCV)
               result = PEDS_WriteRamDataKEY( ADDR_PED_MKEY2_01+(i*PED_MSKEY2_SLOT_LEN2), PED_MSKEY2_SLOT_LEN2, mkey );
               }
             }
	   }
...
UINT  len;
      ...
      if( g_ped_dev == PED_DEV_INT )
        ...
        PEDS_ClearRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, 0 );
...
//                 memset( g_ped_temp, 0x00, sizeof( g_ped_temp ) );
            ...
            case PED_STATE_WAIT_KEY:
                 ...
                 if( (key >= 0x30) && (key <= 0x39) ) // '0'..'9'
                   {
                   PEDS_ReadRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, temp );
                ...
                //   PED_TripleDES( kek, 16, &pin[1], (UCHAR *)&temp[1] ); // encrypted format
                     ...
                     PEDS_WriteRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, temp );
...
//                   g_ped_temp[0] -= 1;
                   ...
                   PEDS_ReadRamDataKEY( ADDR_PEDS_PIN, 1, temp );
                   if( temp[0] != 0 )
                     {
                     temp[0] -= 1;
                     PEDS_WriteRamDataKEY( ADDR_PEDS_PIN, 1, temp );
                     }
...
//                   return( apiFailed ); // invalid length (<4)
                     ...
                     PEDS_ReadRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, pin );
                     PEDS_TripleDES( kek, 16, &pin[1], (UCHAR *)&temp[1] );
                     temp[0] = pin[0];
                     PEDS_WriteRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, temp );
...
//           apiFailed
// ------------------------------------------------------------
UCHAR	PEDS_PutPinPadTMK( UCHAR *etmk, UCHAR index )
{
ULONG	status;
UCHAR	*pSwap;
...
//	  FLASH_ReadData( (void *)FlashAddr+index*PED_MSKEY_SLOT_LEN2, pSwap, DataSize );
	  ...
	  // update ETMK in RAM
	  memmove( pSwap+((F_ADDR_PED_ETMK_01+(index*PED_MSKEY2_SLOT_LEN2))-FLASH_BACKUP_BASE_ADDR), etmk, PED_MSKEY2_SLOT_LEN2 );
...
//           apiFailed
// ------------------------------------------------------------
UCHAR	PEDS_PutPinPadTMK_RAM( UCHAR *etmk, UCHAR cnt )
{
UCHAR	i;
ULONG	status;
ULONG	DataSize = PEDS_FLASH_BACKUP_SIZE;	// 64KB, sizeof FLASH backup area
ULONG	FlashAddr = FLASH_BACKUP_BASE_ADDR;
	...
	if( g_pSwap )
	  ...
	  // update ETMKs in RAM
	  memmove( g_pSwap+(F_ADDR_PED_ETMK_01-FLASH_BACKUP_BASE_ADDR), etmk, cnt*PED_MSKEY2_SLOT_LEN2 );
...
//           apiFailed
// ------------------------------------------------------------
UCHAR	PEDS_PutPinPadTMK_ROM( UCHAR *etmk, UCHAR cnt )
{
ULONG	status;
ULONG	DataSize = PEDS_FLASH_BACKUP_SIZE;	// 64KB, sizeof FLASH backup area
ULONG	FlashAddr = FLASH_BACKUP_BASE_ADDR;;
...
UCHAR tkcv[8];
      ...
      if( index < MAX_MKEY2_CNT )
        {
        // save encrypted key to Flash key slot
        buf[0] = length;
        PEDS_EncryptMKEY( length, key, &buf[1] ); // encrypted by default KEK
        PEDS_PutPinPadTMK( buf, index );

        // save plaintext key to SRAM key slot
        buf[0] = length;
        memmove( &buf[1], key, length );
        PEDS_WriteRamDataKEY( ADDR_PED_MKEY2_01+index*PED_MSKEY2_SLOT_LEN2, PED_MSKEY2_SLOT_LEN2, buf );
        
        // 2018-05-02, setup KCV[3] for api_peds_GetStatus()
        memset( temp, 0x00, 8 );
        PEDS_TripleDES( &buf[1], 8, temp, tkcv );
        PEDS_WriteRamDataKEY( ADDR_PED_MKEY2_01+index*PED_MSKEY2_SLOT_LEN2+PED_MSKEY2_SLOT_LEN, 3, tkcv );
...
// INPUT   : index  - key slot index. (default 0)
//           length - length of master key data. (MUST be 8 or 16)
//           key    - master key data. (eTMK=TDES(bTMK, TMK))
//	     BaseTmkIndex - where the base TMK resides.
// OUTPUT  : none.
// RETURN  : apiOK
//           apiFailed
// ------------------------------------------------------------
UCHAR PEDS_SetMasterKey_HNB( UCHAR BaseTmkIndex, UCHAR index, UCHAR length, UCHAR *key )
{
UCHAR buf[PED_MSKEY2_SLOT_LEN2];
UCHAR temp[8];
UCHAR tkcv[8];

UCHAR tmk[PED_MSKEY2_SLOT_LEN2];
UCHAR ebtmk[PED_MSKEY2_SLOT_LEN2];
UCHAR btmk[PED_MSKEY2_SLOT_LEN2];
ULONG FlashAddr = F_ADDR_PED_ETMK_01;
      ...
      if( index < MAX_MKEY2_CNT )
        {
        // === New for HNB ==========================================================================
        // Get target ebTMK, ebTMK=TDES(bTMK, KEK)
        FLASH_ReadData( (void *)FlashAddr+(BaseTmkIndex*PED_MSKEY2_SLOT_LEN2), ebtmk, PED_MSKEY2_SLOT_LEN2 );
        
        // Restore bTMK by default Key Encryption Key (KEK)
	btmk[0] = ebtmk[0];
	PEDS_DecryptMKEY( &ebtmk[1], &btmk[1] );		// bTMK=L(1)+V(16)
	
	// Restore TMK by bTMK, TMK=~TDES(bTMK, eTMK)
	PEDS_TripleDES2( &btmk[1], length, key, &buf[1] );	// buf=TMK
	memmove( tmk, &buf[1], length );			// tmk=TMK
	memset( buf, 0x00, sizeof(buf) );
	...
	// calculate KCV
        memset( temp, 0x00, 8 );
        PEDS_TripleDES( tmk, 8, temp, tkcv );

        // save encrypted key to Flash key slot
        buf[0] = length;
        PEDS_EncryptMKEY( length, tmk, &buf[1] ); // encrypted by default KEK
        
        memmove( &buf[length+1], tkcv, 3 );	 // including KCV
        PEDS_PutPinPadTMK( buf, index );

        // save plaintext key to SRAM key slot
        buf[0] = length;
        memmove( &buf[1], tmk, length );
        PEDS_WriteRamDataKEY( ADDR_PED_MKEY2_01+index*PED_MSKEY2_SLOT_LEN2, PED_MSKEY2_SLOT_LEN2, buf );
...
//                              <- CMD_SET_MKEY + RC_XXX
// ------------------------------------------------------------
UCHAR api_peds_SetMasterKey_HNB( UCHAR BaseTmkIndex, UCHAR index, UCHAR length, UCHAR *key )
{
UCHAR result;
UCHAR buf[64];
UINT  len;
      ...
      // --- internal PINPAD ---
      if( g_ped_dev == PED_DEV_INT )
        return( PEDS_SetMasterKey_HNB( BaseTmkIndex, index, length, key ) );
      else
        return( apiFailed );
...
{
UCHAR buf[PED_MSKEY2_SLOT_LEN];
UCHAR skey[PED_MSKEY2_SLOT_LEN];
UCHAR mkey[PED_MSKEY2_SLOT_LEN];


      if( index < MAX_SKEY2_CNT )
        {
        // retrive default MKEY
        PEDS_ReadRamDataKEY( ADDR_PED_MKEY2_01+(g_ped_mkey_index*PED_MSKEY2_SLOT_LEN2), PED_MSKEY2_SLOT_LEN2, mkey );
//      TL_DumpHexData(0,0,17,mkey);
        ...
        buf[0] = length;
        memmove( &buf[1], key, length );
        PEDS_TripleDES2( &mkey[1], PED_MSKEY2_LEN, &buf[1], &skey[1] );

        // save it
        skey[0] = length;
        return( PEDS_WriteRamDataKEY( ADDR_PED_SKEY2_01+index*PED_MSKEY2_SLOT_LEN, PED_MSKEY2_SLOT_LEN, skey ) );
        }
      else
        return( apiFailed );
}
...
UCHAR skey[PED_MSKEY2_SLOT_LEN];
      ...
      if( index < MAX_SKEY2_CNT )
        {
        // restore PIN data
        PEDS_ReadRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, temp ); // L-V
        pin[0] = temp[0];
        PEDS_TripleDES2( kek, 16, (UCHAR *)&temp[1], &pin[1] );
//      TL_DumpHexData( 0, 0, 17, pin );
//      TL_DumpHexData( 0, 3, 17, pan );

        // generate plaintext PIN Block
        PEDS_GenPinBlock_ISO0( pin, pan, pinblock );	// same as IPC_server: OS_PED3.c
//      TL_DumpHexData( 0, 0, 8, pinblock );

        // generate EPB (Encrypted PIN Block) by using SKEY
        PEDS_ReadRamDataKEY( ADDR_PED_SKEY2_01+index*PED_MSKEY2_SLOT_LEN, PED_MSKEY2_SLOT_LEN, skey );
//      TL_DumpHexData(0,0,17,skey);

        PEDS_TripleDES( &skey[1], 8, pinblock, epb );
//      TL_DumpHexData(0,3,8, epb);

        PEDS_ClearRamDataKEY( ADDR_PEDS_PIN, PEDS_PIN_SLOT_LEN, 0 ); // clear PIN data
        return( apiOK );
        }
      else
        {
...
UCHAR skey[PED_MSKEY_SLOT_LEN];
      ...
      if( index < MAX_SKEY2_CNT )
        ...
        PEDS_ReadRamDataKEY( ADDR_PED_SKEY2_01+index*PED_MSKEY2_SLOT_LEN, PED_MSKEY2_SLOT_LEN, skey ); // L-V
//      TL_DumpHexData( 0, 0, 17, skey );

        memset( icv, 0x00, 8 ); // icv
//      PED_GenMAC_X9D19( &skey[1], icv, length, data, mac );
        PEDS_GenMAC_BASE24( &skey[1], icv, length, data, mac );			// CTCB algorithm
...
//    api_sys_info( SID_TerminalSerialNumber, tsn );	// LEN(1) + CUST_ID(N3) + SN(N8)
      ...
      for( i=0; i<MAX_MKEY2_CNT; i++ )
         {
         PEDS_ReadRamDataKEY( ADDR_PED_MKEY2_01+i*PED_MSKEY2_SLOT_LEN2, PED_MSKEY2_SLOT_LEN2, key );
//       if( i == 0 )
//         //memmove( kcv, &key[17], sizeof(kcv) );
//           memmove( &kcv[i][0], &key[17], 3 );

         if( key[0] == PED_MSKEY2_LEN ) // PATCH: 2009/01/03
           {
           // verify KCV
           memset( temp, 0x00, 8 );
           PEDS_TripleDES( &key[1], 8, temp, tkcv );
      ...
      // SKEY Status (2)
      skeymap = 0;
      for( i=0; i<MAX_SKEY2_CNT; i++ )
         {
         PEDS_ReadRamDataKEY( ADDR_PED_SKEY2_01+i*PED_MSKEY2_SLOT_LEN, PED_MSKEY2_SLOT_LEN, key );
...
UCHAR dhn_aux;
UCHAR buf[40];
UCHAR mkey[PED_MSKEY2_SLOT_LEN2];  // LEN(1) TMK(16) KCV(3)
UINT  len;
UCHAR dhn_tim;
UINT  tick1, tick2;
...
UCHAR	temp[8];
UCHAR	kcv[8];
UCHAR	etmk[MAX_MKEY2_CNT][PED_MSKEY2_SLOT_LEN2];
UCHAR	tmk_cnt = 0;
...
//UCHAR tkcv[8];
      ...
      for( i=0; i<MAX_MKEY2_CNT; i++ )
         {
         PEDS_ClearRamDataKEY( ADDR_PED_MKEY2_01+(i*PED_MSKEY2_SLOT_LEN2), PED_MSKEY2_SLOT_LEN2, 0x00 );
         PEDS_ClearRamDataKEY( ADDR_PED_SKEY2_01+(i*PED_MSKEY2_SLOT_LEN), PED_MSKEY2_SLOT_LEN, 0x00 );
         etmk[i][0] = 0;
         }
...
//	TL_DumpHexData( 0, 0, 17, mkey );
             ...
             memset( temp, 0x00, 8 );
             PEDS_TripleDES( &mkey[1], 8, temp, kcv );
...
//	TL_DumpHexData( 0, 3, 8, kcv );
             ...
             if( (kcv[0] == buf[2+17]) && (kcv[1] == buf[2+18]) && (kcv[2] == buf[2+19]) )
               ...
               buf[2] = PED_MSKEY2_LEN;
               memmove( &etmk[index][0], &buf[2], PED_MSKEY2_SLOT_LEN2 );
               
               // save plaintext TMK to SRAM key slot
               PEDS_WriteRamDataKEY( ADDR_PED_MKEY2_01+(index*PED_MSKEY2_SLOT_LEN2), PED_MSKEY2_SLOT_LEN2, mkey );


//	       // calculate KCV
//             memset( temp, 0x00, 8 );
//             PED_TripleDES( mkey, 8, temp, tkcv );
        
	       // 2018-05-02, setup KCV[3] for api_peds_GetStatus()
//	       PEDS_WriteRamDataKEY( ADDR_PED_MKEY_01+index*PED_MSKEY_SLOT_LEN2+PED_MSKEY_SLOT_LEN, 3, tkcv );
	       PEDS_WriteRamDataKEY( ADDR_PED_MKEY2_01+index*PED_MSKEY2_SLOT_LEN2+PED_MSKEY2_SLOT_LEN, 3, kcv );	// 2021-02-01
      ...
      if( result == apiOK )
        {
        PEDS_PutPinPadTMK_RAM( &etmk[0][0], tmk_cnt );
        PEDS_PutPinPadTMK_ROM( &etmk[0][0], tmk_cnt );
        }
	  ...
	  switch( mode & 0x0F )
	        ...
	        case DES_MODE_ECB:
	             ...
	             if( mode & DES_MODE_DECRYPT )
	               PEDS_TripleDES2( &skey[1], length, idata, odata );
	             else
	               PEDS_TripleDES( &skey[1], length, idata, odata );
	        ...
	        case DES_MODE_CBC:
	             ...
	             if( mode & DES_MODE_DECRYPT )
	               PEDS_CBC_TripleDES2( &skey[1], icv, length, idata, odata );
	             else
	               PEDS_CBC_TripleDES( &skey[1], icv, length, idata, odata );
